...
*   **زمان‌بندی:** از `zoneinfo` برای مدیریت دقیق زمان UTC و تهران استفاده شده است.
*   **شناسه معامله (Trade ID):** شناسه یکتا برای هر معامله تولید و در تمام پیام‌های مربوط به آن معامله (ورود، تغییر حد ضرر، خروج) حفظ می‌شود.
*   **خط ساختاریافته `TRADE|...`:** این خط در انتهای تمام پیام‌های معاملاتی (ورود، خروج) در تلگرام شخصی اضافه می‌شود تا تحلیل داده در اکسل/پاور کوئری آسان شود.
*   **مدیریت ریسک:** محاسبه حجم پوزیشن بر اساس ریسک درصدی و حد ضرر اولیه، با رعایت سقف حداکثر نوتional پوزیشن، در `trading/risk.py` پیاده‌سازی شده است.
*   **مدیریت پوزیشن:** منطق Break-even و Profit Lock، تشخیص TP/SL با استفاده از High/Low کندل، و مدیریت وضعیت در `trading/engine.py` انجام می‌شود.
*   **گزارش‌دهی:** گزارش‌های ۴ ساعته و ژورنال روزانه با جزئیات کامل (شامل معاملات بسته شده، پوزیشن‌های باز، وضعیت حساب و بازار) به تلگرام شخصی ارسال می‌شوند.
*   **کانال سیگنال:** سیگنال‌ها فقط در زمان‌های مشخص (London/New York) و با قالب ساده‌تر به کانال ارسال می‌شوند. آپدیت‌های حد ضرر و خروج معامله نیز به کانال فرستاده می‌شوند.
*   **News Alerts (Placeholder):** ماژولی برای دریافت اخبار اقتصادی در نظر گرفته شده که فعلاً یک نمونه‌ی ساده (placeholder) است و نیاز به اتصال به API واقعی دارد.
*   **Session Alerts:** هشدارهایی برای شروع و پایان جلسات معاملاتی اصلی (توکیو، لندن، نیویورک) در نظر گرفته شده است.
*   **Render Deployment:** سرور keep-alive برای Render و استفاده از متغیرهای محیطی (`.env`) برای توکن‌ها و تنظیمات پیاده‌سازی شده است.
*   **Error Handling & Logging:** مدیریت خطاها و لاگ‌گیری ساختاریافته در تمام ماژول‌ها لحاظ شده است.
*   **Version Control:** بخش‌هایی برای مدیریت نسخه‌ی بات و استراتژی اضافه شده است.

---

**مرحله بعدی:**

1.  **بررسی و تست:** لطفاً این کدها را بررسی کنید. اگر مشکلی مشاهده کردید یا نیاز به تغییر جزئی دارید، بفرمایید.
2.  **فایل `.env`:** یک فایل `.env` در ریشه‌ی پروژه ایجاد کنید و متغیرهای مورد نیاز (مانند `TELEGRAM_BOT_TOKEN`, `TELEGRAM_PERSONAL_CHAT_ID`, `TELEGRAM_CHANNEL_CHAT_ID`) را در آن قرار دهید.
3.  **نصب وابستگی‌ها:** فایل `requirements.txt` را با دستور `pip freeze > requirements.txt` (پس از نصب پکیج‌های لازم مانند `pandas`, `python-dotenv`, `requests`, `zoneinfo` - که zoneinfo معمولاً در پایتون‌های جدیدتر داخلی است) ایجاد کنید.
4.  **استقرار:** کدها را در ریپو گیت‌هاب خود قرار دهید و Render را برای اجرای آن تنظیم کنید.

پس از این، می‌توانیم به ساخت ربات تلگرام جدید و تنظیمات نهایی بپردازیم.
